...
func _process(_delta: float) -> void:
	var v:float = 0.0
	var direction:Vector2 = Input.get_vector("p1_left","p1_right","p1_up","p1_down")
	var velo: Vector2 = self.velocity
	...
	if direction == Vector2.ZERO:
		if collision != null:
			if call_deferred("self.position.x")== self.position.x:
				velo.x = 0
			if call_deferred("self.position.y")== self.position.y:
				velo.y = 0
	self.velocity = velo
